...
# normalizer = TurkishSentenceNormalizer(morphology) # Render Linux ortamında OverflowError veriyor, iptal edildi
# spell_checker = TurkishSpellChecker(morphology) # Render Linux ortamında OverflowError veriyor, iptal edildi
...
def normalize():
    ...
    return jsonify({'error': 'Normalizer devre dışı bırakıldı (Linux uyumsuzluğu)'}), 500


@app.route('/api/spell-check', methods=['POST'])
def spell_check():
    """Yazım kontrolü ve öneri"""
    return jsonify({'error': 'SpellChecker devre dışı bırakıldı (Linux uyumsuzluğu)'}), 500
